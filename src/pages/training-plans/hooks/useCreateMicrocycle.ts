...
import { createMicrocycle } from '@/service/trainingMicrocycles.service'
import { trainingPlanKeys } from './queryKeys'

const toastId = 'create-microcycle'

export function useCreateMicrocycle(planId: string) {
  const queryClient = useQueryClient()

  const { mutate, isPending } = useMutation({
    mutationFn: createMicrocycle,
